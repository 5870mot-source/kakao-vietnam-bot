...
# 12가지 세부 주제 리스트
TOPICS = [
    "비즈니스 미팅", "이메일 작성", "협상 전략", "발표 스킬", "전화 응대", "피드백 주고받기",
    "여행 회화", "카페 주문", "길 묻기", "병원 방문", "호텔 체크인", "일상 대화"
]
    ...
    try:
        ...
        if not session["level"]:
            if user_message in ["초급", "중급", "고급"]:
                ...
                return {
                    ...
                    "quickReplies": [{"label": t, "action": "message", "messageText": t} for t in TOPICS[:10]] # 카카오 한계 고려 상위 10개 먼저 노출
                }
            else:
                return {
                    "version": "2.0",
        ...
        if not session["topic"]:
            if user_message in TOPICS:
                ...
                return {
                    "version": "2.0",
                    "template": {
                        "outputs": [{"simpleText": {"text": "아래 버튼에서 학습할 주제를 선택해 주세요!"}}],
                        "quickReplies": [{"label": t, "action": "message", "messageText": t} for t in TOPICS[:10]]
                    }
                }
        ...
        if step == 1:
            text = (
                ...
                f"원어민이 대화나 미팅을 시작할 때 가장 자주 사용하는 알짜배기 표현입니다.\n\n"
                f"💬 **Core Expression:**\n"
                f"• *Let's get down to business.* (본론으로 들어갑시다.)\n\n"
                f"💡 **Learning Tip:**\n"
                f"가벼운 인사 후에 분위기를 전환하여 본격적인 논의를 이끌어낼 때 매우 유용하게 쓰입니다."
            )
            ...
            text = (
                ...
                f"실전 업무 및 상황별 대처 능력을 높여주는 전문 심화 표현입니다.\n\n"
                f"💬 **Advanced Vocab:**\n"
                f"• *Mitigate the risk* (위험을 완화하다)\n"
                f"• *Supply chain disruption* (공급망 차단)\n\n"
                f"💡 **Learning Tip:**\n"
                f"잠재적 리스크를 설명하고 상대방을 논리적으로 설득할 때 핵심이 되는 어휘들입니다."
            )
            quick_replies = [
            ...
            text = (
                ...
                f"✍️ 채팅창에 문장을 입력하시면 AI가 즉시 피드백을 드립니다."
            )
            quick_replies = [
                {"label": "⬅️ 이전", "action": "message", "messageText": "이전 단계"},
